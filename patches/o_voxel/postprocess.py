...
def to_glb(
    ...
    # Rasterize the UV atlas with the dependency-free torch implementation.
    rast = uv_raster.rasterize_uv(out_uvs, out_faces, texture_size)

    # Mask of valid pixels in texture
    mask = rast[0, ..., 3] > 0

    # Interpolate 3D positions in UV space (finding 3D coord for every texel)
    pos = uv_raster.interpolate(out_vertices.unsqueeze(0), rast, out_faces)[0][0]
    valid_pos = pos[mask]
