...
## ✅ A100 확인 결과 반영 (2026-07)

A100 서버는 **구버전 T2O(text→TRELLIS)** 상태로, 새 text→image→3D(TRELLIS.2)와 겹치는 게 적다. 실제 결정:

| 분류 | 크기 | 결정 |
|---|---|---|
| A. 코드 (파이프라인+prompt_lab) | ~825MB (대부분 trellis2_src 821M) | ✅ **전송**(A100은 구버전만 있어 덮어써야 함). **압축 번들로 전송** |
| B. 모델 가중치 (FLUX/TRELLIS.2) | ~70GB | ⬇️ **A100에서 HF 재다운로드**(옮기기 무거움. A100엔 대용량 가중치 없음, 게이트 캐시만 있음) |
| C. 입력 데이터 (movie_usd) | — | ❌ **전송 안 함**(A100에 hidden_time 구조 존재. 스모크 검증 가능) |
| D. 문서 | <1MB | ✅ **전송**(A100에 없음) → 압축 번들에 포함 |
| E. env (conda) | — | ❌ 재구축(A100에 `trellis2` env 뼈대 이미 있음 → `A100_HANDOFF §3`으로 검증·보완) |
| F. 생성물/캐시 (runs, RMBG/dinov3/CLIP 캐시) | — | ❌ 전송 안 함(캐시는 A100에 있거나 재다운) |

**→ 실제 압축·전송 대상 = §A 코드 + §D 문서** (movie_usd·모델·env·캐시 제외).
아래 §B~§F 원본 표는 참고용으로 남긴다.
...
## B. 모델 가중치 — ⬇️ A100에서 HF 재다운로드 (전송 안 함)

> A100 확인: 대용량 가중치(FLUX/TRELLIS.2)는 **없음**(HF refs 스텁만), 게이트 캐시(RMBG/dinov3)는 **있음**.
> 옮기기 무거우니 **A100에서 게이트 승인 후 `hf download`** 로 새로 받는다. 아래는 무엇을 받아야 하는지 목록.
...
## C. 입력 데이터 — ❌ 전송 안 함 (A100에 존재)

> A100 확인: `movie_usd/hidden_time` 구조·object USDA(갈매기 등)가 이미 있어 **스모크 검증 가능**.
> (dev는 1.7GB지만 A100은 대용량 캐릭터 에셋이 빠진 경량본 — 풀 씬 검증이 꼭 필요할 때만 선택 동기화.)
